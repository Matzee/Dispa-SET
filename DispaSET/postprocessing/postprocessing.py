...
def plot_dispatch_safe(demand, plotdata, level=None, curtailment=None, rng=None):
    ...
    if "FlowIn" in plotdata and "FlowOut" in plotdata:
        plotdata["FlowIn"], plotdata["FlowOut"] = (
            np.maximum(0, plotdata["FlowIn"] - plotdata["FlowOut"]),
    ...
    for j in range(idx_zero):
        ...
        plt.fill_between(idx, sumplot_neg.loc[pdrng, col1].values, sumplot_neg.loc[pdrng, col2].values, color=color, alpha=alpha, hatch=hatch)
        labels.append(col1)
    ...
    for j in range(len(sumplot_pos.columns) - 1):
        ...
        color = commons["colors"][col2]
        hatch = commons["hatches"][col2]
        plt.fill_between(idx, sumplot_pos.loc[pdrng, col1].values, sumplot_pos.loc[pdrng, col2].values, color=color, alpha=alpha, hatch=hatch)
        labels.append(col2)
        patches.append(mpatches.Patch(color=color, alpha=0.3, hatch=hatch, label=col2))
        colorlist.append(color)
    ...
    if isinstance(curtailment, pd.Series):
        ...
        color = "red"
        plt.fill_between(
            idx, sumplot_neg.loc[pdrng, "sum"].values - curtailment[pdrng].values, sumplot_neg.loc[pdrng, "sum"].values, color="red", alpha=0.7
        )
...
def plot_dispatch(demand, plotdata, level=None, curtailment=None, rng=None):
    ...
    # Netting the interconnections:
    if "FlowIn" in plotdata and "FlowOut" in plotdata:
        plotdata["FlowOut"], plotdata["FlowIn"] = (
            np.minimum(0, plotdata["FlowIn"] + plotdata["FlowOut"]),
            np.maximum(0, plotdata["FlowOut"] + plotdata["FlowIn"]),
        )
    ...
    for j in range(idx_zero):
        ...
        color = commons["colors"][col2]
        hatch = commons["hatches"][col2]
        plt.fill_between(pdrng, sumplot_neg.loc[pdrng, col1], sumplot_neg.loc[pdrng, col2], color=color, alpha=alpha, hatch=hatch)
        labels.append(col1)
        patches.append(mpatches.Patch(color=color, alpha=0.3, hatch=hatch, label=col2))
        colorlist.append(color)
    ...
    for j in range(len(sumplot_pos.columns) - 1):
        ...
        color = commons["colors"][col2]
        hatch = commons["hatches"][col2]
        plt.fill_between(pdrng, sumplot_pos.loc[pdrng, col1], sumplot_pos.loc[pdrng, col2], color=color, alpha=alpha, hatch=hatch)
        labels.append(col2)
        patches.append(mpatches.Patch(color=color, alpha=0.3, hatch=hatch, label=col2))
        colorlist.append(color)
...
def plot_rug(df_series, on_off=False, cmap="Greys"):
    ...
    fig, axes = plt.subplots(nrows=rows, ncols=1, sharex=True, figsize=(16, 0.25 * rows), squeeze=False, frameon=False, gridspec_kw={"hspace": 0.15})
    ...
    for (item, iseries), iax in zip(df_series2.iteritems(), axes.ravel()):
        ...
        if iseries.sum() > 0:
            if on_off:
                ...
                iax.scatter(x, y, marker="|", s=100, c=iseries.values * 100, cmap=cmap)
...
def plot_energy_country_fuel(inputs, results, PPindicators):
    ...
    ax.barh(demand, left=ax.get_xticks() - 0.4, width=[0.8] * len(demand), height=ax.get_ylim()[1] * 0.005, linewidth=2, color="k")
    return ax
...
def get_sim_results(path=".", cache=False, temp_path=".pickle"):
    ...
    for key in [
        ...
        if key in results:
            ...
            elif len(results[key]) == len(index):  # Case of variables for which the look-ahead is not recorded (standard case)
    ...
    if "model" in results["status"]:
        ...
        if len(errors) > 0:
            logging.critical(
                "Some simulation errors were encountered. Some results could not be computed, for example at \n \
                            time "
                + str(errors.index[0])
                + ', with the error message: "'
                + GAMSstatus("model", errors["model"].iloc[0])
                + '". \n \
...
def plot_country(inputs, results, c="", rng=None):
    ...
    if diff.max() > 0.01 * demand.max():
        logging.critical(
            "There is more than 1% difference in the energy balance \
                         of country "
            + c
        )
...
def get_result_analysis(inputs, results):
    ...
    for key in ["LostLoad_RampUp", "LostLoad_2D", "LostLoad_MinPower", "LostLoad_RampDown", "LostLoad_2U", "LostLoad_3U", "LostLoad_MaxPower"]:
        LL = results[key].values.sum()
        if LL > 0.0001 * TotalLoad:
            logging.critical(
                "\nThere is a significant amount of lost load for " + key + ": " + str(LL) + " MWh. The results should be checked carefully"
            )
        elif LL > 100:
            logging.warning("\nThere is lost load for " + key + ": " + str(LL) + " MWh. The results should be checked")
...
def ds_to_df(inputs):
    ...
    if len(dates) > len(sets["h"]):
        logging.error(
            "The provided index has a length of " + str(len(dates)) + " while the data only comprises " + str(len(sets["h"])) + " time elements"
        )
        sys.exit(1)
    elif len(dates) > len(sets["z"]):
        logging.warn(
            "The provided index has a length of "
            + str(len(dates))
...
def CostExPost(inputs, results):
    ...
    for u in CostHeat:
        if u in dfin["CHPPowerLossFactor"].index:
            CostHeat[u] = (
                dfin["CostVariable"][u].fillna(0) * results["OutputHeat"][u].fillna(0) * dfin["CHPPowerLossFactor"].loc[u, "CHPPowerLossFactor"]
            )
        else:
    ...
    costs["LostLoad"] = (
        80e3
        * (
            results["LostLoad_2D"].reindex(timeindex).sum(axis=1).fillna(0)
            + results["LostLoad_2U"].reindex(timeindex).sum(axis=1).fillna(0)
            + results["LostLoad_3U"].reindex(timeindex).sum(axis=1).fillna(0)
        )
        + 100e3
        * (
            results["LostLoad_MaxPower"].reindex(timeindex).sum(axis=1).fillna(0)
            + results["LostLoad_MinPower"].reindex(timeindex).sum(axis=1).fillna(0)
        )
        + 70e3
        * (
            results["LostLoad_RampDown"].reindex(timeindex).sum(axis=1).fillna(0)
