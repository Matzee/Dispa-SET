...
_LOGCONFIG = {
    ...
    "formatters": {
        "standard": {"format": "%(asctime)s [%(levelname)-8s] (%(funcName)s): %(message)s", "datefmt": "%y/%m/%d %H:%M:%S"},
        "notime": {"format": "[%(levelname)-8s] (%(funcName)s): %(message)s", "datefmt": "%y/%m/%d %H:%M:%S"},
    },
    "handlers": {
        "console": {
        ...
        "error_file": {"class": "logging.FileHandler", "level": "WARNING", "formatter": "standard", "filename": "warn.log", "encoding": "utf8"},
    },
