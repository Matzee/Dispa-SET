...
CHPType
COMMITEDInitial(u)              [n.a.]   Initial committment status
Config
...
StorageOutflow(u,h, day)              [MWh\u]    Storage outflows
StorageInflow(u,h, day)               [MWh\u]    Storage inflows (potential energy)
StorageInitial(u, day)                [MWh]    Storage level before initial period
...
$If %RetrieveStatus% == 1 COMMITEDCalc(u,z)               [n.a.]   Committment status as for the MILP
...
StorageFinalMin(s, day)               [MWh]   Minimum storage level at the end of the optimization horizon
;

* Scalar variables necessary to the loop:
scalar FirstHour(day),LastHour(day),LastKeptHour(day),day,ndays,failed;
FirstHour(day) = 1;
...
$LOAD StorageInflow
$LO, dayAD StorageInitial
$, dayLOAD StorageProfile
$LOAD StorageMinimum
$LOAD StorageOutflow
$LO, dayAD Technology
...
$LOAD CostRampUp
$LOAD CostRampDown
$If %RetrieveStatus% == 1 $LOAD COMMITEDCalc
;
...
TimeDownMinimum,
TimeUpMinimum
$If %RetrieveStatus% == 1 , COMMITEDCalc
;
...
*===============================================================================
VARIABLES
COMMITED(u,h)             [n.a.]  Unit COMMITED at hour h {1 0} or integer
STARTUP(u,h)        [n.a.]  Unit start up at hour h {1 0}  or integer
SHUTDOWN(u,h)       [n.a.]  Unit shut down at hour h {1 0} or integer
;

$If %LPFormulation% == 1 POSITIVE VARIABLES COMMITED (u,h) ; COMMITED.UP(u,h) = 1 ;
$If not %LPFormulation% == 1 INTEGER VARIABLES COMMITED (u,h), STARTUP(u,h), SHUTDOWN(u,h) ; COMMITED.UP(u,h) = Nunits(u) ; STARTUP.UP(u,h) = Nunits(u) ; SHUTDOWN.UP(u,h) = Nunits(u) ;
...
COMMITEDInitial(u)=0;
COMMITEDInitial(u)$(PowerInitial(u)>0)=1;
...
$If %Verbose% == 1 Display RampSTARTUPMaximum, RampShutDownMaximum, COMMITEDInitial;
...
EQ_LoadShedding
$If %RetrieveStatus% == 1 EQ_COMMITEDCalc
;
...
EQ_SystemCost(i)..
         SystemCost(i)
         =E=
         sum(u,CostFixed(u)*COMMITED(u,i))
...
EQ_Commitment(u,i)..
         COMMITED(u,i)-COMMITEDInitial(u)$(ord(i) = 1)-COMMITED(u,i-1)$(ord(i) > 1)
         =E=
...
EQ_MinUpTime(u,i)..
         ...
         + sum(h$( (ord(h) >= FirstHour(day) + ord(i) - TimeUpMinimum(u) -1) and (ord(h) < FirstHour(day))),STARTUP.L(u,h))
         =L=
         COMMITED(u,i)
;
...
EQ_MinDownTime(u,i)..
         ...
         + sum(h$( (ord(h) >= FirstHour(day) + ord(i) - TimeDownMinimum(u) -1) and (ord(h) < FirstHour(day))),SHUTDOWN.L(u,h))
         =L=
         Nunits(u)-COMMITED(u,i)
;
...
EQ_RampUp_TC(u,i)$(sum(tr,Technology(u,tr))=0)..
         ...
         =L=
         (COMMITED(u,i) - STARTUP(u,i)) * RampUpMaximum(u) + RampSTARTUPMaximumH(u,i) * STARTUP(u,i) - PowerMustRun(u,i) * SHUTDOWN(u,i) + LL_RampUp(u,i)
;
...
EQ_RampDown_TC(u,i)$(sum(tr,Technology(u,tr))=0)..
         Power(u,i-1)$(ord(i) > 1) + PowerInitial(u)$(ord(i) = 1) - Power(u,i)
         =L=
         (COMMITED(u,i) - STARTUP(u,i)) * RampDownMaximum(u) + RampShutDownMaximumH(u,i) * SHUTDOWN(u,i) - PowerMustRun(u,i) * STARTUP(u,i) + LL_RampDown(u,i)
;
...
EQ_Reserve_2U_capability(u,i)..
         Reserve_2U(u,i)
         =L=
         PowerCapacity(u)*LoadMaximum(u,i)*COMMITED(u,i) - Power(u,i)
;

EQ_Reserve_2D_capability(u,i)..
         Reserve_2D(u,i)
         =L=
         (Power(u,i) - PowerMustRun(u,i) * COMMITED(u,i)) + (StorageChargingCapacity(u)*Nunits(u)-StorageInput(u,i))$(s(u))
;

EQ_Reserve_3U_capability(u,i)$(QuickStartPower(u,i) > 0)..
         Reserve_3U(u,i)
         =L=
         (Nunits(u)-COMMITED(u,i))*QuickStartPower(u,i)
;

*Minimum power output is above the must-run output level for each unit in all periods
EQ_Power_must_run(u,i)..
         PowerMustRun(u,i) * COMMITED(u,i) - (StorageInput(u,i) * CHPPowerLossFactor(u) )$(chp(u) and (CHPType(u,'Extraction') or CHPType(u,'P2H')))
         =L=
         Power(u,i)
;
...
EQ_Storage_input(s,i)..
         StorageInput(s,i)
         =L=
         StorageChargingCapacity(s)*(Nunits(s)-COMMITED(s,i))
;
...
EQ_Storage_MaxDischarge(s,i)..
         ...
         +StorageOutflow(s,i, day)*Nunits(s) +Spillage(s,i) - StorageInflow(s,i, day)*Nunits(s)
         =L=
         StorageLevel(s,i)
;
...
EQ_Storage_balance(s,i)..
         StorageInitial(s, day)$(ord(i) = 1)
         +StorageLevel(s,i-1)$(ord(i) > 1)
*         +StorageLevelH(h--1,s)
         +StorageInflow(s,i, day)*Nunits(s)
         +StorageInput(s,i)*StorageChargingEfficiency(s)
         =E=
         StorageLevel(s,i)
         +StorageOutflow(s,i, day)*Nunits(s)
         +spillage(s,i)
         +Power(s,i)/(max(StorageDischargeEfficiency(s),0.0001))
;
...
* E.g: renewable units with AF>0 must be COMMITED
EQ_Force_Commitment(u,i)$((sum(tr,Technology(u,tr))>=1 and LoadMaximum(u,i)>0))..
         COMMITED(u,i)
         =G=
         1;

* E.g: renewable units with AF=0 must be deCOMMITED
...
EQ_Heat_Storage_balance(chp,i)..
          StorageInitial(c, dayhp)$(ord(i) = 1)
...
Parameter
   resultantx(rh,u, h) 'collector for level of commitment'
   srep(rh, mattrib)  'model attibutes like modelstat etc'
...
failed=0;
parameter COMMITEDInitial_dbg(u), PowerInitial_dbg(u), StorageInitial_dbg(s);

* Fixing the initial guesses:
*PowerH.L(u,i)=PowerInitial(u);
*COMMITED.L(u,i)=COMMITEDInitial(u);
...
FOR(day = 1 TO ndays-Config("RollingHorizon LookAhead","day") by Config("RollingHorizon Length","day"),
        ...
        FirstHour(day) = (day-1)*24+1;
        LastHour(day) = min(card(h), FirstHour(day) + (Config("RollingHorizon Length", "day") + Config("RollingHorizon LookAhead", "day")) * 24 - 1);
        LastKeptHour(day) = LastHour(day) - Config("RollingHorizon LookAhead","day") * 24;
        i(h) = no;
        i(h)$(ord(h) >= FirstHour(day) and ord(h) <= LastHour(day))=yes;
*       display day, FirstHour(day), LastHour(day), LastKeptHour(day);

*       Defining the minimum level at the end of the horizon, ensuring that it is feasible with the provided inflows:
        StorageFinalMin(s, day) =  min(StorageInitial(s, day) + 
                        (sum(i,StorageInflow(s, i, day)) - sum(i,StorageOutflow(s,i, day)))*Nunits(s),
...
*       Correcting the minimum level to avoid the infeasibility in case it is too close to the StorageCapacity:
        StorageFinalMin(s, day) = min(StorageFinalMin(s, day),Nunits(s)*StorageCapacity(s) - Nunits(s)*smax(i,StorageInflow(s, i, day)));


        COMMITEDInitial(u) = sum(i$(ord(i)=LastKeptHour(day)-FirstHour(day)+1),COMMITED.L(u,i));
        PowerInitial(u) = sum(i$(ord(i)=LastKeptHour(day)-FirstHour(day)+1),Power.L(u,i));

        StorageInitial(s, day) = sum(i$(ord(i)=LastKeptHour(day)-FirstHour(day)+1), StorageLevel.L(s,i));
        StorageInitial(c, dayhp) = sum(i$(ord(i)=LastKeptHour(day)-FirstHour(day)+1), StorageLevel.L(chp,i));


*       Loop variables to display after solving:
*        $If %Verbose% == 1 Display LastKeptHour(day),PowerInitial,CostStartupH.L,CostShutDownH.L,CostRampUpH.L;
*        PARAMETER elapsed(day); elapsed(day) = (jnow - starttime)*24*3600;
);
...
*$If %Verbose% == 1   Display PowerInitial,COMMITEDInitial,StorageFinalMin;

Set dict    /   day. rollinghorizon. ''
...
if(UCM_SIMPLE.Modelstat <> 1 and UCM_SIMPLE.Modelstat <> 8 and not failed, COMMITEDInitial_dbg(u) = COMMITEDInitial(u); PowerInitial_dbg(u) = PowerInitial(u); StorageInitial_dbg(s) = StorageInitial(s, day);
                                                                            EXECUTE_UNLOAD "debug.gdx" day, status, COMMITEDInitial_dbg, PowerInitial_dbg, StorageInitial_dbg;
                                                                            failed=1;);


CurtailedPower.L(n,z)=sum(u,(Nunits(u)*PowerCapacity(u)*LoadMaximum(u,z)-Power.L(u,z))$(sum(tr,Technology(u,tr))>=1) * Location(u,n));

$If %Verbose% == 1 Display Flow.L,Power.L,COMMITED.L,ShedLoad.L,CurtailedPower.L,StorageLevel.L,StorageInput.L,SystemCost.L,LL_MaxPower.L,LL_MinPower.L,LL_2U.L,LL_2D.L,LL_RampUp.L,LL_RampDown.L;
...
OutputCOMMITED(u,z)=COMMITED.L(u,z);
...
EXECUTE 'GDXXRW.EXE "%inputfilename%" O="Results.xlsx" Squeeze=Y par=StorageInflow rng, day=StorageInflow!A1 , dayrdim=1 cdim=1'
EXECUTE 'GDXXRW.EXE "%inputfilename%" O="Results.xlsx" Squeeze=N par=StorageCapacity rng=StorageCapacity!A1 rdim=1 cdim=0'
...
EXECUTE 'GDXXRW.EXE "Results.gdx" O="Results.xlsx" Squeeze=N par=OutputCOMMITED rng=COMMITED!A1 rdim=1 cdim=1'
...
$LOAD COMMITEDInitial_dbg
$LOAD StorageInitial_dbg
;
PowerInitial(u) = PowerInitial_dbg(u); COMMITEDInitial(u) = COMMITEDInitial_dbg(u); StorageInitial(s, day) = StorageInitial_dbg(s);
FirstHour(day) = (day-1)*24+1;
LastHour(day) = min(card(h),FirstHour(day) + (Config("RollingHorizon Length","day")+Config("RollingHorizon LookAhead","day")) * 24 - 1);
LastKeptHour(day) = LastHour(day) - Config("RollingHorizon LookAhead","day") * 24;
i(h) = no;
i(h)$(ord(h)>=FirstHour(day) and ord(h)<=LastHour(day))=yes;
StorageFinalMin(s, day) =  min(StorageInitial(s, day) + sum(i,StorageInflow(s,i, day)) - sum(i,StorageOutflow(s,i, day)) , sum(i$(ord(i)=card(i)),StorageProfile(s,i)*StorageCapacity(s)*AvailabilityFactor(s,i)));
StorageFinalMin(s, day) = min(StorageFinalMin(s, day),StorageCapacity(s) - smax(i,StorageInflow(s,i, day)));
$If %Verbose% == 1   Display TimeUpLeft_initial,TimeUpLeft_JustStarted,PowerInitial,COMMITEDInitial,StorageFinalMin;
$If %LPFormulation% == 1          SOLVE UCM_SIMPLE USING LP MINIMIZING SystemCostD;
$If not %LPFormulation% == 1      SOLVE UCM_SIMPLE USING MIP MINIMIZING SystemCostD;
...
display day,FirstHour(day),LastHour(day),LastKeptHour(day);
Display StorageFinalMin,PowerInitial,COMMITEDInitial,StorageFinalMin;
Display Flow.L,Power.L,COMMITED.L,ShedLoad.L,StorageLevel.L,StorageInput.L,SystemCost.L,Spillage.L,StorageLevel.L,StorageInput.L,LL_MaxPower.L,LL_MinPower.L,LL_2U.L,LL_2D.L,LL_RampUp.L,LL_RampDown.L;
