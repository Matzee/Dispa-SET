...
def is_sim_folder_ok(sim_folder):
    ...
    if not os.path.exists(os.path.join(sim_folder, u"Inputs.gdx")):
        logging.error(
            "There is no Inputs.gdx file within the specified DispaSET simulation environment folder ("
            + sim_folder
            + "). Check that the GDX output is activated in the option file and that no error stated during the pre-processing"
        )
        return False
