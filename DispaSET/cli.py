...
@click.option(
    "-c",
    "--config",
    prompt="Specify the path to the config file",
    type=click.Path(exists=True),
    default="./ConfigFiles/ConfigTest.xlsx",
    help="Path to the config file (eg ConfigFiles/Config.xlsx)",
)
@click.option(
    "-g", "--gams", "engine", flag_value="gams", default=True, help="Use GAMS version (default)"
)  # Interface to use for solving. Gams or pyomo')
@click.option("-p", "--pyomo", "engine", flag_value="pyomo", help="Use pyomo version")
...
def build(ctx):
    ...
    if engine == "gams" and conf["WriteGDX"]:
        logging.warning(
            "The config specifies that a gdx file should be written, although PYOMO is selected as engine. This a properly installed version of GAMS. Desactivate the option if it is not the case"
        )
