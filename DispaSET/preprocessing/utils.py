...
def interconnections(Simulation_list, NTC_inter, Historical_flows):
    ...
    if len(index) == 0:
        ...
        logging.warn(
            "The two input dataframes (NTCs and Historical flows) do not share the same index, although some values are common. The intersection has been considered and "
            + str(diff)
    ...
    if (NTC_inter.values < 0).any():
        ...
        logging.warn(
            "WARNING: At least NTC value is negative, for example in line "
            + str(NTC_inter.columns[pos[1][0]])
            + " and time step "
            + str(NTC_inter.index[pos[0][0]])
        )
    if (Historical_flows.values < 0).any():
        pos = np.where(Historical_flows.values < 0)
        logging.warn(
            "WARNING: At least one historical flow is negative, for example in line "
            + str(Historical_flows.columns[pos[1][0]])
            + " and time step "
    ...
    while compare_set:
        ...
        for name in connNames:
            if nameToCompare[0:2] in name[0:2]:
                ...
                logging.info(
                    "Detected interconnection "
                    + name
                    + ", happening between a simulated zone and the rest of the world. The historical flows will be imposed to the model"
                )
                ...
                logging.info(
                    "Detected interconnection "
                    + name
                    + ", happening between the rest of the world and a simulated zone. The historical flows will be imposed to the model"
                )
