...
from .data_handler import UnitBasedTable, NodeBasedTable, merge_series, define_parameter, write_to_excel, load_csv, set_sets
...
def build_simulation(config, plot_load=False):
    ...
    # updating stopdate to the end of the day
    config["StopDate"] = (y_end, m_end, d_end, 23, 59, 00)

    # Load fuel types, technologies, timestep, etc:
    commons = commonvars()

    # Indexes of the simualtion:
    idx_std = pd.DatetimeIndex(start=pd.datetime(*config["StartDate"]), end=pd.datetime(*config["StopDate"]), freq=commons["TimeStep"])
    idx_utc_noloc = idx_std - dt.timedelta(hours=1)
    # idx_utc = idx_utc_noloc.tz_localize("UTC") #-> NOT USED
    ...
    # delta = idx_utc[-1] - idx_utc[0] #-> NOT USED
    # days_simulation = delta.days + 1 #-> NOT USED
    # hours_simulation = 24 * days_simulation #-> NOT USED
    ...
    # Load Shedding:
    LoadShedding = NodeBasedTable(
        config["LoadShedding"], idx_utc_noloc, config["countries"], tablename="LoadShedding", default=config["default"]["LoadShedding"]
    )
    CostLoadShedding = NodeBasedTable(
        config["CostLoadShedding"], idx_utc_noloc, config["countries"], tablename="CostLoadShedding", default=config["default"]["CostLoadShedding"]
    )
    ...
    AF = UnitBasedTable(
        ...
        tablename="AvailabilityFactors",
        default=1,
        RestrictWarning=commons["tech_renewables"],
    )
    ReservoirLevels = UnitBasedTable(
        plants_sto,
        config["ReservoirLevels"],
        idx_utc_noloc,
        config["countries"],
    ...
    ReservoirScaledInflows = UnitBasedTable(
        plants_sto,
        config["ReservoirScaledInflows"],
        idx_utc_noloc,
        config["countries"],
        fallbacks=["Unit", "Technology", "Zone"],
    ...
    HeatDemand = UnitBasedTable(
        plants_chp, config["HeatDemand"], idx_utc_noloc, config["countries"], fallbacks=["Unit"], tablename="HeatDemand", default=0
    )
    CostHeatSlack = UnitBasedTable(
        plants_chp,
        config["CostHeatSlack"],
        idx_utc_noloc,
        config["countries"],
        fallbacks=["Unit", "Zone"],
        tablename="CostHeatSlack",
    ...
    check_df(AF_merged, StartDate=idx_utc_noloc[0], StopDate=idx_utc_noloc[-1], name="AF_merged")
    ...
    # Define the sets and the params for further processing
    sets, sets_param = set_sets(Nhours_long, config, Plants_merged, Interconnections, commons, Plants_sto, Plants_chp)
    ...
    for var in ["AvailabilityFactor", "Efficiency", "Curtailment", "StorageChargingEfficiency", "StorageDischargeEfficiency", "Nunits"]:
    ...
    for (i, s) in enumerate(sets["s"]):
        if s in ReservoirLevels_merged:
            # get the time
            parameters["StorageInitial"]["val"][i] = (
                ReservoirLevels_merged[s][idx_long[0]] * Plants_sto["StorageCapacity"][s] * Plants_sto["Nunits"][s]
            )
    ...
    for unit in range(Nunits):
        ...
        if Plants_merged["Fuel"][unit] == "BIO":
            parameters["CostVariable"]["val"][unit, :] = FuelPrices["PriceOfBiomass"] / Plants_merged["Efficiency"][unit]
            found = True
        if not found:
            logging.warn(
                "No fuel price value has been found for fuel "
                + Plants_merged["Fuel"][unit]
                + " in unit "
                + Plants_merged["Unit"][unit]
    ...
    if LP:
        ...
        shutil.copyfile(os.path.join(GMS_FOLDER, "UCM_h.gms"), os.path.join(sim, "UCM_h.gms"))
    gmsfile = open(os.path.join(sim, "UCM.gpr"), "w")
    gmsfile.write(
        "[PROJECT] \n \n[RP:UCM_H] \n1= \n[OPENWINDOW_1] \nFILE0=UCM_h.gms \nFILE1=UCM_h.gms \nMAXIM=1 \nTOP=50 \nLEFT=50 \nHEIGHT=400 \nWIDTH=400"
    )
    gmsfile.close()
    shutil.copyfile(os.path.join(GMS_FOLDER, "writeresults.gms"), os.path.join(sim, "writeresults.gms"))
    ...
    shutil.copy(os.path.join(GMS_FOLDER, "makeGDX.bat"), os.path.join(sim, "makeGDX.bat"))
    ...
    if plot_load:
        ...
        # x_ticks = np.linspace(0, N - 1, 20, dtype="int32") # not used
