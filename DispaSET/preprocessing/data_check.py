    ...
    for t in ["WTON", "WTOF", "PHOT", "HROR"]:
        for i in plants[plants["Technology"] == t].index:
            ...
            if u in AF:
                ...
                logging.critical(
                    "Unit " + str(u) + " (technology " + t + ") does not appear in the availbilityFactors table. Its values will be set to 100%!"
                )
...
def check_clustering(plants, plants_merged):
    ...
    for i in techs.index:
        ...
        if np.abs(P_old - P_new) / (P_old + 0.0001) > 0.01:
            logging.error(
                'The installed capacity for technology "'
                + tech[0]
                + '" and fuel "'
                + tech[1]
                + '" is not equal between the original units table (P = '
                + str(P_old)
                + ") and the clustered table (P = "
                + str(P_new)
                + ")"
            )
            sys.exit(1)
    ...
    logging.info("MIst jetzt erst hier")
    ...
    if np.abs(TotalStorage - TotalStorage_merged) / (TotalStorage + 0.0001) > 0.01:
        logging.error(
            "The total installed storage capacity is not equal between the original units table ("
            + str(TotalStorage)
            + ") and the clustered table ("
            + str(TotalStorage_merged)
            + ")"
        )
        # sys.exit(1)
    return True
...
def check_MinMaxFlows(df_min, df_max):
    ...
    if (df_min > df_max).any():
        ...
        logging.critical(
            "ERROR: At least one minimum flow is higher than the maximum flow, for example in line number "
            + str(pos[0][0])
            + " and time step "
            + str(pos[1][0])
        )
    ...
    if (df_max < 0).any():
        ...
        logging.critical(
            "ERROR: At least one maximum flow is negative, for example in line number " + str(pos[0][0]) + " and time step " + str(pos[1][0])
        )
        sys.exit(1)
...
def check_sto(config, plants, raw_data=True):
    ...
    if "StorageInitial" in plants:
        logging.warn(
            'The "StorageInitial" column is present in the power plant table, although it is deprecated (it should now be defined in the ReservoirLevel data table). It will not be considered.'
        )
...
def check_chp(config, plants):
    ...
    for u in plants.index:
        ...
        if plants.loc[u, "CHPType"].lower() not in ["extraction", "back-pressure", "p2h"]:
            logging.critical(
                'The value of CHPType should be "extraction", "back-pressure" or "p2h". The type of unit '
                + u
                + ' is "'
                + str(plants.loc[u, "CHPType"] + '"')
            )
            sys.exit(1)
        if 0 > plant_powertoheat > 10:
            logging.critical(
                "The value of CHPPowerToHeat should be higher or equal to zero and lower than 10. Unit "
                + u
                + " has a value of "
                + str(plant_powertoheat)
            )
            sys.exit(1)
        if 0 > plant_powerlossfactor > 1 and plants.loc[u, "CHPType"].lower() != "p2h":
            logging.critical(
                "The value of CHPPowerLossFactor should be higher or equal to zero and lower than 1. Unit "
                + u
                + " has a value of "
                + str(plant_powerlossfactor)
            )
            sys.exit(1)
        if plants.loc[u, "CHPType"].lower() == "back-pressure" and plant_powerlossfactor != 0:
            logging.critical(
                'The value of CHPPowerLossFactor must be zero if the CHP types is "back-pressure". Unit '
                + u
                + " has a value of "
                + str(plant_powerlossfactor)
            )
        ...
        if plants.loc[u, "CHPType"].lower() == "extraction":
            ...
            if not pd.isnull(plant_MaxHeat):
                if intersection_MaxHeat < plant_MaxHeat:
                    logging.warning(
                        "Given Maximum heat CHPMaxHeat ({}) is higher than the intersection point of the two other constraints ({}) "
        ...
        if plants.loc[u, "CHPType"].lower() != "p2h":
            ...
            if TotalEfficiency < 0 or TotalEfficiency > 1.14:
                logging.critical(
                    "The calculated value of the total CHP efficiency for unit "
                    + unitname
                    + " is "
                    + str(TotalEfficiency)
            ...
            if TotalEfficiency > 0.95:
                logging.warn(
                    "The calculated value of the total CHP efficiency for unit " + unitname + " is " + str(TotalEfficiency) + ", which is very high!"
                )
        ...
        for u in plants.index:
            ...
            if plants.loc[u, "STOCapacity"] < Qdot * 0.5:
                logging.warn(
                    "Unit "
                    + unitname
                    + ": The value of the thermal storage capacity ("
                    + str(plants.loc[u, "STOCapacity"])
                    + "MWh) seems very low compared to its thermal power ("
                    + str(Qdot)
                    + "MW)."
                )
                ...
                logging.warn(
                    ...
                    + ": The value of the thermal storage capacity ("
                    + str(plants.loc[u, "STOCapacity"])
                    + "MWh) seems very high compared to its thermal power ("
                    + str(Qdot)
                    + "MW)."
                )
        ...
        for u in plants.index:
            if plants.loc[u, "STOSelfDischarge"] < 0:
                logging.error(
                    "Unit "
                    + unitname
                    + ": The value of the thermal storage self-discharge ("
                    + str(plants.loc[u, "STOSelfDischarge"] * 100)
...
def check_units(config, plants):
    ...
    NonNaNKeys = ["PowerCapacity", "PartLoadMin", "RampUpRate", "RampDownRate", "Efficiency", "RampingCost", "CO2Intensity"]
    ...
    if len(plants["Unit"].unique()) != len(plants["Unit"]):
        ...
        logging.error(
            "The names of the power plants are not unique. The following names are duplicates: "
            + str(duplicates)
            + '. "'
            + str(duplicates[0] + '" appears for example in the following countries: ' + str(plants.Zone[plants["Unit"] == duplicates[0]].tolist()))
        )
        sys.exit(1)
    ...
    for key in lower:
        if any(plants[key] < lower[key]):
            ...
            logging.critical(
                "The value of " + key + " should be higher or equal to zero. A negative value has been found for units " + str(plantlist)
            )
            sys.exit(1)
    ...
    for key in lower_hard:
        if any(plants[key] <= lower_hard[key]):
            ...
            plantlist = plantlist["Unit"].tolist()
            logging.critical(
                "The value of " + key + " should be strictly higher than zero. A null or negative value has been found for units " + str(plantlist)
            )
            sys.exit(1)
    ...
    for key in higher:
        if any(plants[key] > higher[key]):
            ...
            plantlist = plantlist["Unit"].tolist()
            logging.critical("The value of " + key + " should be lower or equal to one. A higher value has been found for units " + str(plantlist))
            sys.exit(1)
    ...
    for key in higher_time:
        if any(plants[key] >= config["HorizonLength"] * 24):
            ...
            logging.critical(
                "The value of "
                + key
                + " should be lower than the horizon length ("
                + str(config["HorizonLength"] * 24)
                + " hours). A higher value has been found for units "
                + str(plantlist)
            )
            sys.exit(1)
    ...
    for u in data:
        if u in plants.index:
            ...
            if np.isnan(Qmax) and plant_CHP_type != "p2h":
                ...
                logging.warn(
                    "The maximum thermal demand for unit "
                    + str(u)
                    + " ("
                    + str(data[u].max())
                    + ") is higher than its thermal capacity ("
                    + str(Qmax)
                    + "). Slack heat will be used to cover that."
                )
            if data[u].min() < Qmin:
                logging.warn(
                    "The minimum thermal demand for unit "
                    + str(u)
                    + " ("
                    + str(data[u].min())
                    + ") is lower than its minimum thermal generation ("
                    + str(Qmin)
                    + " MWth)"
                )
        else:
...
def check_df(df, StartDate=None, StopDate=None, name=""):
    ...
    if not df.columns.is_unique:
        logging.error(
            'The column headers of table "' + name + '" are not unique!. The following headers are duplicated: ' + str(df.columns.get_duplicates())
        )
        sys.exit(1)
    return True
...
def check_simulation_environment(SimulationPath, store_type="pickle", firstline=7):
    ...
    list_sets = ["h", "d", "mk", "n", "c", "p", "l", "f", "s", "t", "tr", "u"]
